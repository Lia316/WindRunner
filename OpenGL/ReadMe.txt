...
SceneNode.cpp & h : SceneNode와 SceneGraph를 구현한 파일이다. SceneGraph에 SceneNode를 추가 삭제 접근할 수 있고, SceneGraph를 통해 모든 객체를 그릴 수 있다.
Model.cpp & h : obj 파일을 읽어 draw 할 수 있는 구조체로 변환하는 class의 파일이다.
Material.cpp & h : 3D mesh를 첫 실행에 읽고 객체에게 주소값을 전달해주는 파일이다.
